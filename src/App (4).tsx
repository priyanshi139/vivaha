...
import { AppState, UserType, UserDetails, WeddingDetails, Task, Vendor, InspirationBoard, Blog, RealWedding, Guest, RSVPStatus, RelationType, AdminVendor, AdminBooking, AdminFeedback, AdminReports } from './types';
...
const ExpertChatModal: React.FC<{
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { id: expertMsgId, role: 'expert', text: response.text || "I'm sorry, I couldn't process that. How else can I help?" }]);
...
const AdminLogin: React.FC<{ onLogin: (email: string) => void }> = ({ onLogin }) => {
...
const AdminDashboard: React.FC<{ username: string; onLogout: () => void }> = ({ username, onLogout }) => {
  const [activeTab, setActiveTab] = useState<'vendors' | 'bookings' | 'feedback' | 'reports' | 'customers'>('reports');
  ...
  const fetchData = async () => {
    try {
      ...
      } else if (activeTab === 'bookings' || activeTab === 'customers') {
  ...
  const respondToFeedback = async (id: string, response: string) => {
    await fetch(`/api/admin/feedback/${id}/respond`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  // Group bookings by customer for the "Customers" view
  const customers = Array.from(new Set(bookings.map(b => b.customer_name))).map(name => ({
    name,
    bookings: bookings.filter(b => b.customer_name === name)
  }));

  return (
    <div className="absolute inset-0 bg-slate-50 flex flex-col z-[200]">
      <div className="p-6 bg-slate-800 text-ivory flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold">Admin Dashboard</h2>
          <p className="text-[10px] text-slate-400 uppercase tracking-widest">Logged in as {username}</p>
        </div>
      ...
      <div className="flex bg-white border-b border-slate-200 overflow-x-auto no-scrollbar">
        {['reports', 'vendors', 'bookings', 'customers', 'feedback'].map((tab) => (
          <button
            key={tab}
            onClick={() => setActiveTab(tab as any)}
            className={`px-6 py-4 text-[10px] font-bold uppercase tracking-widest transition-all whitespace-nowrap ${activeTab === tab ? 'text-rose border-b-2 border-rose' : 'text-slate-400'}`}
          >
            {tab}
          </button>
        ))}
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
        {activeTab === 'reports' && reports && (
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Total Bookings</p>
              <p className="text-3xl font-bold text-slate-800">{reports.totalBookings}</p>
            </div>
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Pending Vendors</p>
              <p className="text-3xl font-bold text-rose">{reports.pendingApprovals}</p>
            </div>
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 col-span-2">
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Approved Vendors</p>
              <p className="text-3xl font-bold text-emerald-500">{reports.approvedVendors}</p>
            </div>
          </div>
        )}
        ...
        {activeTab === 'vendors' && (
          <div className="space-y-4">
            <h3 className="text-lg font-bold text-slate-800 mb-2">Vendor Applications</h3>
            {vendors.length > 0 ? vendors.map(v => (
              <div key={v.id} className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100">
                <div className="flex justify-between items-start mb-4">
                  <div>
                    <h4 className="font-bold text-slate-800">{v.name}</h4>
                    <p className="text-xs text-slate-500">{v.services}</p>
                  </div>
                  <span className={`text-[10px] font-bold px-2 py-1 rounded-full uppercase ${v.status === 'approved' ? 'bg-emerald-100 text-emerald-600' : v.status === 'rejected' ? 'bg-red-100 text-red-600' : 'bg-amber-100 text-amber-600'}`}>
                    {v.status}
                  </span>
                </div>
                <div className="flex gap-2">
              ...
              <p className="text-center text-slate-400 py-10">No vendors found.</p>
            )}
          </div>
        )}
        ...
        {activeTab === 'bookings' && (
          <div className="space-y-4">
            <h3 className="text-lg font-bold text-slate-800 mb-2">All Bookings</h3>
            {bookings.length > 0 ? bookings.map(b => (
              <div key={b.id} className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100">
                <div className="flex justify-between items-start mb-2">
                  <h4 className="font-bold text-slate-800">{b.service}</h4>
                  <span className="text-[10px] font-bold text-slate-400">{b.date}</span>
                </div>
                <p className="text-sm text-slate-600 mb-1">Customer: {b.customer_name}</p>
                <p className="text-xs text-slate-400">Status: {b.payment_status}</p>
              </div>
            )) : (
              <p className="text-center text-slate-400 py-10">No bookings found.</p>
            )}
          </div>
        )}
        ...
        {activeTab === 'customers' && (
          <div className="space-y-4">
            <h3 className="text-lg font-bold text-slate-800 mb-2">Customer List</h3>
            {customers.length > 0 ? customers.map(c => (
              <div key={c.name} className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100">
                ...
                <div className="space-y-2">
                  <p className="text-[10px] font-bold text-slate-400 uppercase">Bookings ({c.bookings.length})</p>
                  {c.bookings.map(b => (
                    <div key={b.id} className="flex justify-between items-center text-xs py-2 border-t border-slate-50">
              ...
              <p className="text-center text-slate-400 py-10">No customers found.</p>
            )}
          </div>
        )}
        ...
        {activeTab === 'feedback' && (
          <div className="space-y-4">
            <h3 className="text-lg font-bold text-slate-800 mb-2">Customer Feedback</h3>
            {feedback.length > 0 ? feedback.map(f => (
              <div key={f.id} className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100">
                <div className="flex justify-between items-start mb-2">
                  <h4 className="font-bold text-slate-800">{f.customer_name}</h4>
                  <span className="text-[10px] text-slate-400">{new Date(f.created_at).toLocaleDateString()}</span>
                </div>
                <p className="text-sm text-slate-600 mb-4 italic">"{f.message}"</p>
                {f.response ? (
                  <div className="bg-slate-50 p-3 rounded-xl border border-slate-100">
                    <p className="text-[10px] font-bold text-slate-400 uppercase mb-1">Admin Response</p>
                  ...
                  <div className="flex gap-2">
                    <input 
                      ...
                      className="flex-1 bg-slate-50 border border-slate-100 rounded-xl px-3 py-2 text-xs outline-none focus:border-rose/30"
                      onKeyPress={(e) => {
                        if (e.key === 'Enter') respondToFeedback(f.id, (e.target as HTMLInputElement).value);
                      }}
                    />
                    <button 
                      onClick={(e) => {
                        const input = (e.currentTarget.previousSibling as HTMLInputElement);
                        if (input.value) respondToFeedback(f.id, input.value);
                      }}
                      className="bg-rose text-white px-4 py-2 rounded-xl text-xs font-bold"
                    >
                      Send
                    </button>
                  </div>
              ...
              <p className="text-center text-slate-400 py-10">No feedback found.</p>
            )}
          </div>
        )}
      </div>
        ...
        <div className="flex gap-2 overflow-x-auto no-scrollbar">
          {['dashboard', 'bookings', 'profile'].map((tab) => (
            <button 
              key={tab}
              onClick={() => setActiveTab(tab as any)}
              className={`px-6 py-2 rounded-full text-xs font-bold capitalize transition-all ${activeTab === tab ? 'bg-rose text-ivory' : 'bg-white text-slate-500 border border-slate-100'}`}
            >
              {tab}
            </button>
          ))}
        ...
        {activeTab === 'dashboard' && (
          <div className="space-y-4">
            <div className="flex justify-between items-center">
              ...
              <button className="text-xs font-bold text-rose">View All</button>
            </div>
            {[
              { name: 'Priya & Rahul', date: 'Dec 12, 2026', status: 'New' },
              { name: 'Amit & Sneha', date: 'Jan 05, 2027', status: 'Pending' },
            ].map((inquiry, i) => (
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input type="text" defaultValue="Royal Palace Weddings" className="w-full p-4 bg-white rounded-xl border border-slate-100 outline-none focus:border-gold" />
              </div>
              <div>
                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2 block">Description</label>
                <textarea defaultValue="A premier wedding venue in the heart of Udaipur..." className="w-full p-4 bg-white rounded-xl border border-slate-100 outline-none focus:border-gold h-32" />
              </div>
...
const PlannerDashboard: React.FC<{ onLogout: () => void, onLaunchAi: () => void }> = ({ onLogout, onLaunchAi }) => {
        ...
        {activeTab === 'ai_tools' && (
          <div className="space-y-6">
            <div className="bg-gradient-to-br from-slate-800 to-slate-900 p-8 rounded-[2.5rem] text-ivory relative overflow-hidden">
              ...
              <button 
                onClick={onLaunchAi}
                className="bg-gold text-slate-900 px-6 py-3 rounded-xl font-bold text-xs"
              >
                Launch AI Assistant
              </button>
            </div>
...
export default function App() {
              ...
              <div className="bg-ivory w-full rounded-t-[3rem] p-8 max-h-[80vh] overflow-y-auto">
                ...
                {loadingAi ? (
                  <div className="py-12 flex flex-col items-center gap-4">
                    ...
                    <p className="text-slate-500 italic">Consulting with wedding experts...</p>
                  </div>
                ) : (
                  <div className="space-y-6">
                    <div className="prose prose-slate max-w-none">
                      <div className="whitespace-pre-wrap text-slate-700 leading-relaxed bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
                        {aiResponse || "Hello! I'm your AI Wedding Planner. How can I help you today?"}
                      </div>
                    </div>
          ...
          <div className="space-y-6 bg-white p-6 rounded-[2rem] shadow-sm border border-slate-50">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-4">
                ...
                <div className="relative">
                  <Calendar className="absolute left-4 top-1/2 -translate-y-1/2 text-rose" size={18} />
                  <input 
                    type="date" 
                    value={filterDate}
                    onChange={(e) => setFilterDate(e.target.value)}
                    className="w-full bg-slate-50 border border-slate-100 rounded-2xl pl-12 pr-4 py-3 text-sm outline-none focus:border-rose/30 transition-colors text-slate-600"
                  />
                </div>
              </div>
            </div>
  ...
  const GuestListPage = () => {
    ...
    const sendInvite = (guest: Guest, platform: 'whatsapp' | 'sms' | 'email') => {
      const couple = "Priya & Rohan";
      const date = "Dec 12, 2026";
  ...
  const BudgetTab = () => {
    return (
      <div className="pb-24">
        <BudgetOverview userType={state.userType as 'bride' | 'groom'} />
      </div>
    );
  };
  ...
  return (
    <div className="w-full max-w-md mx-auto h-screen bg-ivory shadow-2xl relative overflow-x-hidden flex flex-col box-border">
      <AnimatePresence mode="wait">
        ...
        {!showSplash && state.screen === 'admin_dashboard' && state.adminUser && <AdminDashboard key="admin_dashboard" username={state.adminUser} onLogout={() => setState(prev => ({ ...prev, screen: 'user_type', adminUser: undefined }))} />}
        ...
        {!showSplash && state.screen === 'planner_dashboard' && <PlannerDashboard key="planner_dashboard" onLogout={() => setState(prev => ({ ...prev, screen: 'user_type' }))} onLaunchAi={() => setIsExpertChatOpen(true)} />}
